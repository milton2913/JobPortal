<?php

use Illuminate\Database\Seeder;
use App\Models\Institute;
class InstituteTableSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {


        Institute::create( [
            'name'=>'Ahsanullah University of Science and Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'America Bangladesh University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'American International University-Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Asa University Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Asian University of Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Atish Dipankar University of Science and Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Balakhal J. N. High School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangabandhu Sheikh Mujibur Rahman Agricultural University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangabandhu Sheikh Mujibur Rahman Maritime University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangabandhu Sheikh Mujibur Rahman Science and Technology University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh Agricultural University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh Institute of Science & Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh Islami University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh National University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh Open University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh Technical Education Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh University of Business and Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh University of Engineering and Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh University of Professionals',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bangladesh University of Textiles',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Barisal Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Begum Gulchemonara Trust University Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Begum Rokeya University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bhatkhali A Karim High School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bhedergonj HeadQuarter Pilot High School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bhor Bazar Advocate Belayet Hossain High School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Bogra Polytechnic Institute',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'BRAC University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'BUET',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Cantonment Board High School, Mymensingh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Cantonment Public School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Central Womens University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Chapai Nawabganj Polytechnic Institute',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Chittagong Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Chittagong Independent University (CIU)',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Chittagong University of Engineering & Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Chittagong Veterinary and Animal Sciences University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'City University, Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Comilla Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Comilla University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Coxs Bazar International University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Daffodil International University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Darul Ihsan University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Dhaka Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Dhaka International University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Dhaka University of Engineering & Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Dinajpur Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Dinajpur Institute of Science',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Dinajpur Polytechnic Institute, Dinajpur',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Dr. Mahabubur Rahman mollah College',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'DU',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'East Delta University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'East West University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Eastern University, Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'European University of Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Feni Polytechnic Institute, Feni',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Ghior D N Pilot High School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Gono Bishwabidyalay',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Govt Debendra College',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Govt. Girls High School Faridpur',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Govt. Sarda Sundori Mahila College',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Govt. tolaram collage.',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Green University of Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Hajee Mohammad Danesh Science & Technology University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Harimohan Govt High School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'IBAIS University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Independent University, Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'International Islamic University, Chittagong',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'International University of Business Agriculture and Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Jagannath University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Jahangirnagar University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Jatiya Kabi Kazi Nazrul Islam University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Jessore Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Jessore Polytechnic Institute',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Jessore University of Science & Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'JU',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Kalagachia Union High School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Khulna University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Khulna University of Engineering & Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Khwaja Yunus Ali University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Lalmatia Girl\'s College',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Leading University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Madrasah Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Manarat International University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Mawlana Bhashani Science and Technology University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Memorial University of Newfoundland',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Metropolitan University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Millennium University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Muhammadpur Central College',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Naogaon Govt. College',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'National University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Noakhali Science and Technology University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'North South University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'North Western University, Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Northern University, Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Notre Dame University Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'OSMANIA UNIVERSITY',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Others',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Pabna University of Science and Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Patuakhali Science and Technology University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Peoples University of Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Pirganj Govt. High School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Port City International University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Premier University, Chittagong',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Presidency University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Prime University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Primeasia University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Queens University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Rajshahi Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Rajshahi University of Engineering & Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Rangpur Government College',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Royal University of Dhaka',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Savar College',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Shah Abdur Rouf College',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Shahjalal University of Science and Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Shamsul Hoque Khan School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Shanto-Mariam University of Creative Technology',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Sher-e-Bangla Agricultural University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Shishu Niketon High School, Rangpur',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'SONARGAON DEGREE COLLEGE',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Sonargaon University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'South Sathkania GolamBari High School',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Southeast University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Southern University, Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Stamford University Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'State University of Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'SUST',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Syamnagor Mohasin Degree College',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Sylhet Agricultural University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Sylhet Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Sylhet International University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Technical Board',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'testinsti1',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'United International University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of Asia Pacific (Bangladesh)',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of Barisal',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of Chittagong',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of Development Alternative',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of Dhaka',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of Greenwich',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of Information Technology and Sciences',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of Liberal Arts Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of Rajshahi',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of Science & Technology Chittagong',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'University of South Asia, Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Uttara University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Varendra University',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'Victoria University of Bangladesh',
            'country_id'=>'18'
        ] );



        Institute::create( [
            'name'=>'World University of Bangladesh',
            'country_id'=>'18'
        ] );
        

    }
}
